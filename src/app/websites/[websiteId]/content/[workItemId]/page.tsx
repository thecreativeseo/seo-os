...
import { getDraftForWorkItem, listDraftsForWorkItem } from "@/server/services/content-draft";
...
export default async function ContentWorkItemPage({
  ...
  const supersededDrafts = drafts.filter((row) => row.status === "SUPERSEDED");
          ...
          <div className="border-border space-y-2 rounded-lg border p-4">
            ...
            {draftView.briefMismatch ? (
              <p className="text-sm">
                <span className="font-medium">
                  This draft was created from Brief v{draftView.brief.version}. Brief v
            ...
            {supersededDrafts.length > 0 ? (
              <p className="text-muted-foreground text-xs">
                {supersededDrafts.length} earlier draft{supersededDrafts.length === 1 ? "" : "s"}{" "}
                superseded and still inspectable:{" "}
                {supersededDrafts.map((row, index) => (
                  <span key={row.id}>
                    {index > 0 ? ", " : ""}
                    <Link
                      href={`/websites/${websiteId}/content/${item.id}/draft?draft=${row.id}`}
                      className="hover:underline"
                    >
